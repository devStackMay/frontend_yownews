...
export async function updateCategory(session: AppSession, id: string, body: { nom: string; description?: string }) {
  // KSM attend `description` en paramètre de requête (@RequestParam), pas dans le corps JSON —
  // et n'accepte pas de renommer (`nom` n'est lu par aucun paramètre de l'endpoint côté KSM).
  const res = await callKsm<Response>(
    `/api/v1/newsletter/categorie/${id}?description=${encodeURIComponent(body.description ?? '')}`,
    { method: 'PUT', raw: true },
    { session },
  );
  return readRaw<CategorieEntity>(res);
...
export async function unsubscribeCategory(session: AppSession, userId: string, categorieId: string) {
  const res = await callKsm<Response>(`/api/v1/newsletter/abonnements/categories/${categorieId}?userId=${userId}`, { method: 'DELETE', raw: true }, { session });
  await readRaw<void>(res);
}
